                                    ...
                                    <div class="col-sm-12 float-left pb-2 px-3">
                                        <div class="input-group mb-3">
                                            ...
                                            <input type="text" v-model="carikata" class="form-control form-control-sm"
                                                placeholder="Masukkan Kata Kunci Pencarian Nama Unit Organisasi"
                                                @keyup="ambildata()">
                                        </div>
                                    </div>
...
export default {
    ...
    methods: {
        ...
        async ambildata() {
            ...
            var random = Math.random();
            await mainAPI.get("parastapainnovationunor-GetData?random=" + random + "&halaman=" + this.halaman + "&cari=" + this.carikata).then(
                Response => {
                    this.datapesan = Response.data.message;
                    if (this.datapesan == 'data diketemukan') {
                        this.datamaster = [];
        ...
        prosesdata(kode) {
            ...
            window.location.replace("/ProsesUnor/" + bytes);
        },
        hapus(kode) {
            swal.fire({
                title: "Konfirmasi",
                ...
                if (result.isConfirmed) {
                    ...
                    mainAPI.post("parastapainnovationunor-Delete", data).then(
                        Response => {

                ...
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6 wow fadeInDown">
                            <h5>Master Pilihan Jawaban</h5>
                        </div>
                        <div class="col-sm-6 wow fadeInDown">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="/Dashboard">Home </a></li>
                                <li class="breadcrumb-item active">Master Pilihan Jawaban</li>
                            </ol>
                        </div>
                    </div><!-- /.row -->
                </div><!-- /.container-fluid -->
                                        ...
                                        <table class="table m-0" id="tabeldata">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="width: 100px;">Pilihan Jawaban</th>
                                                    <th style="width: 150px;">Nilai</th>
                                                    <th style="width: 150px;">Penjelasan Jawaban</th>
                                            ...
                                            <template v-for="(datalist, urutlist) in filteredKataData" :key="urutlist">
                                                <tbody>
                                                    <tr>
                                                        <td>{{ urutlist + 1 }}</td>
                                                        <td>
                                                            {{ datalist.pilihan_jawaban }}
                                                        </td>
                                                        <td>
                                                            {{ datalist.nilai }}
                                                        </td>
                                                        <td>
                                                            <span v-html="datalist.penjelasan_jawaban"></span>
                                                        </td>
                                                        <td class="text-center">
                                                            <button type="button"
                                                                class="btn btn-primary3 btn-sm lebar2 text-white"
                                                                @click="prosesdata(datalist.id_jawaban)"
                                                            ...
                                                            <button type="button"
                                                                ...
                                                                @click="hapus(datalist.id_jawaban)"
                                                                data-toggle="tooltip" data-placement="left"
                                                                title="Hapus data..">
...
export default {
    ...
    methods: {
        ...
        async ambildata() {
            ...
            await mainAPI.get("masterlke-GetAllPilihanJawaban?random=" + random + "&level=" + this.leveluser).then(
                Response => {
        ...
        prosesdata(kode) {
            ...
            window.location.replace("/ProsesPilihanJawaban/" + bytes);
        },
        hapus(kode) {
            swal.fire({
                title: "Konfirmasi",
                ...
                if (result.isConfirmed) {
                    ...
                    mainAPI.post("masterlke-DeletePilihanJawaban", data).then(
                        Response => {

                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-6 wow fadeInDown">
                            <ol class="breadcrumb float-sm-right">
                                ...
                                <li class="breadcrumb-item active">Tim Evaluator</li>
                            </ol>
                        </div>
                    </div><!-- /.row -->
                </div><!-- /.container-fluid -->
                                    ...
                                    <div class="col-sm-12 float-left pb-2 px-3">
                                        <div class="input-group mb-3">
                                            ...
                                            <input type="text" v-model="carikata" class="form-control form-control-sm"
                                                placeholder="Masukkan Kata Kunci Pencarian Nama" @keyup="ambildata()">
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <table class="table m-0" id="tabeldata">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="width: 100px;">Nama Tim Evaluator</th>
                                                    <th style="width: 100px;">Jabatan</th>
                                                    <th style="width: 100px;">Wilayah</th>
                                                    <th style="width: 50px;">Tahun Evaluasi</th>
                                            ...
                                            <template v-for="(datalist, urutlist) in datamaster" :key="urutlist">
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            {{ datalist.tahun_evaluasi }}
                                                        </td>
                                                        <td class="text-center">
                                                            <button type="button"
                                                                class="btn btn-primary3 btn-sm lebar2 text-white"
                                                                @click="prosesdata(datalist.id_evaluator)"
                                                            ...
                                                            <button type="button"
                                                                ...
                                                                @click="hapus(datalist.id_evaluator)"
                                                                data-toggle="tooltip" data-placement="left"
                                                                title="Hapus data..">
...
export default {
    ...
    methods: {
        ...
        async pindahhalaman(halamannya) {
            ...
            await mainAPI.get("masterlke-GetAllEvaluator?random=" + random + "&iduser=" + this.kode_satker + "&level=" + this.leveluser + "&halaman=" + halamannya + "&cari=" + this.carikata).then(
                Response => {
        ...
        async ambildata() {
            ...
            var random = Math.random();
            await mainAPI.get("masterlke-GetAllEvaluator?random=" + random + "&iduser=" + this.kode_satker + "&level=" + this.leveluser + "&halaman=" + this.halaman + "&cari=" + this.carikata).then(
                Response => {
                    this.datapesan = Response.data.message;
                    if (this.datapesan == 'data diketemukan') {
                        this.datamaster = [];
        ...
        prosesdata(kode) {
            ...
            window.location.replace("/ProsesEvaluator/" + bytes);
        },
        hapus(kode) {
            swal.fire({
                title: "Konfirmasi",
                ...
                if (result.isConfirmed) {
                    ...
                    mainAPI.post("masterlke-DeleteEvaluator", data).then(
                        Response => {

                                        ...
                                        <table class="table m-0" id="tabeldata">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="width: 150px;">Unor</th>
                                                    <th style="width: 150px;">Balai</th>
                                            ...
                                            <template v-for="(datalist, urutlist) in datamaster" :key="urutlist">
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            {{ datalist.nama_kategori_satker }}
                                                        </td>
                                                        <td>
                                                            {{ datalist.kode_satker }}
                                                        </td>
                                                        <td>
                                                            {{ datalist.nama_satker }}
                                                        </td>
                                                        <td>
                                                            {{ datalist.namastatus_piu }}
                                                        </td>
                                                        <td>
                                                            {{ datalist.namalevel_piu }}
                                                        </td>
                                                        <td class="text-center">
                                                            <i class="fas fa-check-circle text-success"
                                                                v-if="datalist.aktif == 0"></i>
...
export default {
    ...
    methods: {
        ...
        async pindahhalaman(halamannya) {
            ...
            await mainAPI.get("parastapainnovationsatker-GetData?random=" + random + "&halaman=" + halamannya + "&cari=" + this.carikata).then(
                Response => {
        ...
        async ambildata() {
            ...
            await mainAPI.get("parastapainnovationsatker-GetData?random=" + random + "&halaman=" + this.halaman + "&cari=" + this.carikata).then(
                Response => {
                    this.datapesan = Response.data.message;
                    if (this.datapesan == 'data diketemukan') {
                        this.datamaster = [];
        ...
        prosesdata(kode) {
            var bytes = encodeURIComponent(CryptoJS.AES.encrypt(kode + '|' + this.halaman, 'GPMop8LQ06S0rZXcJyEH3wk8jVrINbHwn7tBq2').toString());
            window.location.replace("/ProsesSatker/" + bytes);
        },
        hapus(kode) {
            ...
            swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    mainAPI.post("parastapainnovationsatker-Delete", data).then(
                        Response => {
                            if (Response.data.response == 'error') {
                                ...
                                swal.fire('Informasi', 'Data telah dihapus..', 'success').then(function () {
                                    window.location.replace("/Satker/" + param);
                                });
                            }
                        }
                    ).catch(
    ...
    mounted() {
        ...
        this.halaman = this.secretencData;
        this.ambildata();
    }
}
</script>

                ...
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6 wow fadeInDown">
                            <h5>Resume Capaian</h5>
                        </div>
                        <div class="col-sm-6 wow fadeInDown">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="/Dashboard">Beranda </a></li>
                                <li class="breadcrumb-item active">Resume Capaian</li>
                            </ol>
                        </div>
                    </div><!-- /.row -->
                </div><!-- /.container-fluid -->
                                ...
                                <table class="table m-0" id="tabeldata" style="font-size: x-small;">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width: 200px;" v-if="showIndikator" rowspan="3">UNIT ORGANISASI /
                                                SASARAN
                                            ...
                                            <th style="width: 150px;" v-if="showOuput" colspan="4">OUTPUT</th>
                                            <th style="width: 60px;" v-if="showKodeUnique" rowspan="2">KINERJA</th>
                                            <th style="width: 150px;" v-if="showOutcome" colspan="4">OUTCOME</th>
                                            <th style="width: 60px;" v-if="showKodeUnique" rowspan="2">KINERJA</th>
                                        </tr>
                                        <tr>
                                            <th colspan="2">
                                    ...
                                    <template v-for="(datalist, urutlist) in filteredKataData" :key="urutlist">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td :class="datalist.class" v-if="showIndikator" colspan="11">
                                                    <a href="javascript:void(0)" data-toggle="tooltip"
                                                        v-if="datalist.kode_unique.includes('IKSK')"
                                                        data-placement="left" title="Lihat Detail.."
                                                        @click="bukadetailindikator(datalist.id, datalist.hitungan_pk, datalist.namasatuan, datalist.namaoutput, datalist.namaoutcome,)">
                                            ...
                                            <template v-if="datalist.level = 'IKSK'"
                                                ...
                                                <tr>
                                                    ...
                                                    <td colspan="12" class="levelbalai">
                                                        {{ databalai.balai }}
                                                    </td>
                                                </tr>
                                                <template v-if="datalist.level = 'IKSK'"
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-center" v-if="showOutcome">
                                                            <span>{{ datasatker.kinerja_outcome }}</span>
                                                        </td>
                                                    </tr>
                                                    <template v-if="datalist.level = 'IKSK'"
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-center" v-if="showOutcome">
                                                                <span>{{ dataemon.kinerja_outcome }}</span>
                                                            </td>
                                                        </tr>
                                                    </template>
                                                </template>
    ...
    methods: {
        ...
        bukadetailindikator(id, hitungan_pk, namasatuan, namaoutput, namaoutcome) {
            ...
            if (this.hitungan_pk == 1) {
                ...
                mainAPI2.get("DetailIndikator-HitunganTarget?random=" + random + "&id_indikator=" + this.id + "&kode_satker=" + kode_satker + "&kategorisatker=" + this.selectedkategorisatker).then(
                    Response => {
                ...
                this.halamanloading = true;
                var random = Math.random();
                mainAPI2.get("DetailIndikator-HitunganSubKomponen?random=" + random + "&id_indikator=" + this.id + "&kode_satker=" + kode_satker + "&kategorisatker=" + this.selectedkategorisatker).then(
                    Response => {
                        this.datapesan = Response.data.message;
                        if (this.datapesan == 'data diketemukan') {
                            this.datadetail = [];
        ...
        bukadetail(id, namasatuan, namaoutput, namaoutcome, hitungan_pk, textindikator, jumlahkomponen, kolomkomponen, bobotkomponen, rumuskomponen, kode_satker, nama_satker, nama_balai) {
            ...
            if (this.hitungan_pk == 1) {
                ...
                this.halamanloading = true;
                var random = Math.random();
                mainAPI2.get("DetailResume-HitunganTarget?random=" + random + "&id_indikator=" + this.id + "&kode_satker=" + this.kode_satker).then(
                    Response => {
                        this.datapesan = Response.data.message;
                        if (this.datapesan == 'data diketemukan') {
                            this.datadetail = [];
                ...
                this.halamanloading = true;
                var random = Math.random();
                mainAPI2.get("DetailResume-HitunganSubKomponen?random=" + random + "&id_indikator=" + this.id + "&kode_satker=" + this.kode_satker).then(
                    Response => {
                        this.datapesan = Response.data.message;
                        if (this.datapesan == 'data diketemukan') {
                            this.datadetail = [];
        ...
        async eksportdetailindikator1(namefile) {
            ...
            var paramurl = "";
            paramurl = "EksportExcelIndikator-Hitungan?random=" + random + "&id_indikator=" + this.id + "&kode_satker=" + kode_satker + "&kategorisatker=" + this.selectedkategorisatker;
        ...
        async eksportdetailindikator2(namefile) {
            ...
            const filename = this.G_formatDate(Date(), 'd-M-Y') + '_' + namefile + '.xlsx';
            var paramurl = "";
            paramurl = "EksportExcelIndikator-Detail2?random=" + random + "&id_indikator=" + this.id + "&kode_satker=" + kode_satker + "&kategorisatker=" + this.selectedkategorisatker;
            await mainAPI5.get(paramurl).then(
                response => {
                    // Buat link download dari blob
                    const url = window.URL.createObjectURL(new Blob([response.data]));
            ...
            if (this.selectedkategorisatker == '' || this.selectedsatker == '') {
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        const filename = this.G_formatDate(Date(), 'd-M-Y') + '_' + namefile + '.xlsx';
                        var paramurl = "";
                        paramurl = "EksportExcel-Satker?random=" + random + "&tahun=" + this.selectedtahun + "&kategorisatker=" + this.selectedkategorisatker + "&kode_satker=" + kode_satker;
        ...
        async eksportdetailhitungan2(namefile) {
            ...
            const filename = this.G_formatDate(Date(), 'd-M-Y') + '_' + namefile + '.xlsx';
            var paramurl = "";
            paramurl = "EksportExcel-Detail2?random=" + random + "&id_indikator=" + this.id + "&kode_satker=" + this.kode_satker;
            await mainAPI5.get(paramurl).then(
                response => {
                    // Buat link download dari blob
                    const url = window.URL.createObjectURL(new Blob([response.data]));
        ...
        async eksportdetailhitungan1(namefile) {
            ...
            const filename = this.G_formatDate(Date(), 'd-M-Y') + '_' + namefile + '.xlsx';
            var paramurl = "";
            paramurl = "EksportExcel-Hitungan?random=" + random + "&id_indikator=" + this.id + "&kode_satker=" + this.kode_satker;
            await mainAPI5.get(paramurl).then(
                response => {
                    // Buat link download dari blob
                    const url = window.URL.createObjectURL(new Blob([response.data]));
            ...
            if (this.selectedkategorisatker == '' && this.selectedsatker == '') {
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        var random = Math.random();
                        mainAPI.get("Resume-Satker?random=" + random + "&tahun=" + this.selectedtahun + "&kategorisatker=" + this.selectedkategorisatker + "&kode_satker=" + kode_satker).then(
                            Response => {
                                this.datapesan = Response.data.message;
                                if (this.datapesan == 'data diketemukan') {
                                    this.datamaster = [];
                ...
                var random = Math.random();
                await mainAPI.get("Resume-Satker?random=" + random + "&tahun=" + this.selectedtahun + "&kategorisatker=" + this.selectedkategorisatker + "&kode_satker=" + kode_satker).then(
                    Response => {
                        this.datapesan = Response.data.message;
                        if (this.datapesan == 'data diketemukan') {
                            this.datamaster = [];

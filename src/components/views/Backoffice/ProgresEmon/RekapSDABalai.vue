                ...
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6 wow fadeInDown">
                            <h5>Progres SDA / Balai</h5>
                        </div>
                        <div class="col-sm-6 wow fadeInDown">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="/Dashboard">Beranda </a></li>
                                <li class="breadcrumb-item active">Progres SDA / Balai</li>
                            </ol>
                        </div>
                    </div><!-- /.row -->
                </div><!-- /.container-fluid -->
                                ...
                                <div class="card-body px-3" style="font-size: smaller !important;">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-6" style="display: none;">
                                            <label>Total Perhitungan Progres Dengan Sumber Dana PHLN</label>
                                            <div class="form-group">
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input"
                                                        id="customSwitchPHLN" @click="hitungdenganPHLN()">
                                                    <label class="custom-control-label" for="customSwitchPHLN">Masukkan
                                                        Perhitungan Dengan Sumber Dana PHLN</label>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="col-sm-12 text-center wow fadeInDown">
                            <a class="hitam">
                                <button type="button" class="btn btn-success btn-sm text-white lebar2"
                                    ...
                                    @click="G_tableCSVTabel1('_RekapBalaiSDA_')">
                                        ...
                                        <table class="table m-0" id="tabeldata" style="font-size: x-small;">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="width: 150px;" rowspan="2">Balai</th>
                                            ...
                                            <template v-for="(datalist, urutlist) in filteredKataData" :key="urutlist">
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        <td :class="datalist.class" class="text-right" data-z="#,##0"
                                                            :data-v="datalist.pagurpm + datalist.pgsbsn + datalist.pgpln"
                                                            v-if="phln == true">
                                                            {{ G_numFormat(datalist.pagurpm + datalist.pgsbsn +
                                                            datalist.pgpln) }}
                                                        </td>
                                                        <td :class="datalist.class" class="text-right" data-z="#,##0"
                                                            :data-v="datalist.pagurpm + datalist.pgsbsn" v-else>
                                                            {{ G_numFormat(datalist.pagurpm + datalist.pgsbsn) }}
                                                        </td>
                                                        <td :class="datalist.class" class="text-right" data-z="#,##0"
...
export default {
    ...
    methods: {
        ...
        hitungdenganPHLN() {
            this.phln = document.getElementById('customSwitchPHLN').checked;
            this.ambildata();
        },
        async G_tableCSVTabel1(namefile) {
            this.halamanloading = true;
            const XLSX = xlsx;
            var wb = XLSX.utils.table_to_book(document.getElementById('tabeldata'), { sheet: "RekapBalaiSDA" });
        ...
        lihatdata(kodesatker) {
            ...
            window.location.replace("/RekapSatker/" + bytes);
        },
        async AmbilDataReff() {
        ...
        async ambildata() {
            ...
            var random = Math.random();
            await mainAPI.get("progres_emon-GetBalaiSDA?random=" + random + "&tahun=" + this.selectedtahun + "&phln=" + this.phln).then(
                Response => {
